...
/** Full Quran surah index with ayah counts for progress dropdowns. */
export const SURAHS: Surah[] = [
  { number: 1, name: 'Al-Fatihah', ayahCount: 7 },
  { number: 2, name: 'Al-Baqarah', ayahCount: 286 },
  { number: 3, name: 'Aal-E-Imran', ayahCount: 200 },
  ...
  { number: 32, name: 'As-Sajda', ayahCount: 30 },
...
export function surahLabel(surah: Surah): string {
  return `${surah.number}. Surah ${surah.name}`;
}
...
export function formatSurahName(number: number): string {
  const surah = getSurah(number);
  return surah ? `Surah ${surah.name}` : `Surah ${number}`;
}
